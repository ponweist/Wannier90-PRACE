...
module w90_berry
  ...
  public :: berry_main,get_imfgh_k_list
...
!                                   get_SS_R
    ...
    if(eval_kubo .and. spin_decomp)  call get_SS_R
             ...
             do if=1,nfermi
                ...
                if(rdum>berry_curv_adpt_kmesh_thresh) then
                   ...
                   do loop_adpt=1,berry_curv_adpt_kmesh**3
                      ...
                      call get_imfgh_k_list(kpt(:)+adkpt(:,loop_adpt),&
                           imf_k_list_dummy)
    ...
    if(present(img_k_list)) then
      ...
      do i=1,3
        ...
        do ife=1,nfermi
          ...
          call gemm(f_list(:,:,ife),    AA(:,:,beta_A(i)), tmp(:,:,1))
          call gemm(AA(:,:,alpha_A(i)), tmp(:,:,1),        tmp(:,:,2))
          call gemm(HH,                 tmp(:,:,2),        tmp(:,:,1))
          ...
          !
          ! J2 term
          call gemm(HH, JJp_list(:,:,ife,beta_A(i)), tmp(:,:,1))
    ...
    if(present(imh_k_list)) then
      ...
      do ife=1,nfermi
        do i=1,3
          ...
          !
          ! J1 term
          call gemm(AA(:,:,alpha_A(i)), JJp_list(:,:,ife,beta_A(i)), tmp(:,:,1))
          s = utility_im_tr_prod(HH, tmp(:,:,1))
          call gemm(JJm_list(:,:,ife,alpha_A(i)), AA(:,:,beta_A(i)), tmp(:,:,1))
          s = s + &
              utility_im_tr_prod(HH, tmp(:,:,1))
          imh_k_list(2,i,ife) = -2.0_dp * s
          !
          ! J2 term
          call gemm(JJm_list(:,:,ife,alpha_A(i)), &
