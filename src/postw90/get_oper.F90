...
101 call io_error('Error in get_HH_R: problem opening file '//&
  ...
  !======================================================!
    ...
    ! Real-space position matrix elements are read from file
    !
    if(effective_model) then
       AA_R=cmplx_0
       if(on_root) then
...
if(on_root) then
    ...
    allocate(S(num_wann,num_wann))
       ...
       deallocate(AA_q,AA_q_diag,S_o,S)
endif !on_root
...
104 call io_error&
  ...
  !============================================================!
    ...
    if (allocated(BB_R)) then
       if(timing_level>1.and.on_root) call io_stopwatch('get_oper: get_BC_R',2)
       return
    end if
...
110 call io_error&
  ...
  !======================================================!
  !                                                      !
  ! Calculates the real space matrices when needed for   !
  ! the required tasks and deallocates v_matrix          !  
  !                                                      !
  !======================================================!

  use w90_parameters, only     : spin_moment, spin_decomp, dos, geninterp, berry, kpath, kslice, &
                                 berry_task, kpath_task, kslice_task, &
                                 kpath_bands_colour, kslice_fermi_lines_colour 
  use w90_postw90_common, only : v_matrix
  ...
  if(dos) then
     call get_HH_R
     if (spin_decomp) call get_SS_R
  endif  

  if(geninterp) call get_HH_R
  ...
  if(berry) then
     if( index(berry_task,'morb')>0 ) then
        ...
        call get_AA_R
        call get_BC_R
     elseif( index(berry_task,'ahc')>0) then
        call get_HH_R
        call get_AA_R
     elseif( index(berry_task,'kubo')>0 ) then
        call get_HH_R
        call get_AA_R
        if (spin_decomp) call get_SS_R
     endif 
  ...
  if(kpath) then
     if( index(kpath_task,'morb')>0 ) then
        ...
        call get_AA_R
        call get_BC_R
     elseif( index(kpath_task,'curv')>0 ) then
        call get_HH_R
        call get_AA_R  
     elseif( index(kpath_task,'bands')>0 ) then
        call get_HH_R  
        if( kpath_bands_colour=='spin' ) call get_SS_R
     endif 
  endif
  ...
  if(kslice) then
     if( index(kslice_task,'morb')>0 ) then
        ...
        call get_AA_R
        call get_BC_R
     elseif( index(kslice_task,'curv')>0 ) then
        call get_HH_R
        call get_AA_R
     elseif( index(kslice_task,'fermi_lines')>0 ) then
        call get_HH_R
        if( kslice_fermi_lines_colour=='spin' ) call get_SS_R
     endif
  endif
